...
from .serializers import UserRegisterSerializer, LoginSerializer, DatasetSerializer, ValidationRuleSerializer
...
def get_tokens_for_user(user):
    ...
    # Adding 'sub' claim to match FastAPI exact payload optionally, but let's stick to simplejwt defaults
...
class RegisterView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            return Response({"access_token": access_token, "token_type": "bearer"}, status=status.HTTP_201_CREATED)
...
class LoginView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if user is not None:
                access_token = get_tokens_for_user(user)
                return Response({"access_token": access_token, "token_type": "bearer"}, status=status.HTTP_200_OK)
